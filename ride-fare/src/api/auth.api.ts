...
export const authApi = {
  ...
  verifyOtp: (payload: { code: string }) => http.post<AuthSession>("/auth/verify-otp", payload),
  resendOtp: () => http.post<{ sent: boolean }>("/auth/resend-otp"),
  forgotPassword: (payload: { email: string }) =>
    http.post<{ sent: boolean; email: string }>("/auth/forgot-password", payload),
  resetPassword: (payload: { token?: string; password: string }) =>
    http.post<{ updated: boolean }>("/auth/reset-password", payload),
  refresh: (payload: { refreshToken: string }) => http.post<AuthTokens>("/auth/refresh", payload),
